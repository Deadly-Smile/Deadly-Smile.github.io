...
async function fileToPayload(file) {
  ...
  return { type: "file", name: file.name, mime: file.type, size: file.size, data: buffer };
}
    ...
    <div style={{
      ...
      <div style={{ maxWidth: "72%", display: "flex", flexDirection: "column", gap: 2, alignItems: isMe ? "flex-end" : "flex-start" }}>
        ...
        <span style={{ fontSize: 11, color: "#9ca3af", paddingInline: 4 }}>
          {formatTime(msg.ts)}
          {isMe && (
            <span style={{ marginLeft: 4, color: msg.delivered ? "#22c55e" : "#d1d5db" }}>
              {msg.delivered ? "✓✓" : "✓"}
            </span>
          )}
        </span>
...
function TypingIndicator() {
  return (
    <div style={{ display: "flex", alignItems: "flex-end", gap: 8, marginBottom: 4 }}>
      ...
      <div style={{ padding: "10px 14px", borderRadius: "16px 16px 16px 4px", background: "#f3f4f6", display: "flex", gap: 4, alignItems: "center" }}>
        {[0, 1, 2].map(i => (
          <div key={i} style={{
...
export default function P2PChat() {
  ...
  const sendTypingSignal = (value) => {
    connRef.current?.open && connRef.current.send({ type: "typing", value });
  };

  const handleInputChange = (e) => {
    setMsgInput(e.target.value);
    if (!myTypingRef.current) { myTypingRef.current = true; sendTypingSignal(true); }
    clearTimeout(typingTimerRef.current);
    typingTimerRef.current = setTimeout(() => { myTypingRef.current = false; sendTypingSignal(false); }, 1500);
  };
  ...
  const sendFile = async (file) => {
    setFileError("");
    if (file.size > MAX_FILE_SIZE) { setFileError(`Max file size is ${formatBytes(MAX_FILE_SIZE)}.`); return; }
    if (!connRef.current?.open) return;
    const id = Date.now();
    const payload = { ...(await fileToPayload(file)), id };
  ...
  if (view === "home") return (
    <div style={s.page}>
      ...
      <div style={s.card}>
        ...
        <div style={{ marginTop: 20, padding: "12px 14px", background: "#f9fafb", borderRadius: 10, border: "0.5px solid #e5e7eb" }}>
  ...
  return (
    <div style={s.chatContainer}>
      ...
      <div style={s.inputBar}>
        ...
        <input style={{ ...s.input, flex: 1 }} placeholder="Message…" value={msgInput}
          onChange={handleInputChange} onKeyDown={e => e.key === "Enter" && !e.shiftKey && sendMsg()} />
...
const anim = `@keyframes bounce { 0%,80%,100%{transform:translateY(0)} 40%{transform:translateY(-6px)} }`;
...
const s = {
  ...
  inputBar: { display: "flex", gap: 8, alignItems: "center", padding: "10px 12px", borderTop: "0.5px solid #e5e7eb" },
  title: { margin: "0 0 4px", fontSize: 20, fontWeight: 600, color: "#111827" },
  muted: { margin: 0, fontSize: 13, color: "#6b7280", lineHeight: 1.5 },
  input: { padding: "9px 13px", borderRadius: 9, border: "0.5px solid #d1d5db", fontSize: 14, outline: "none", background: "#fff", color: "#111827" },
  btnPrimary: { padding: "9px 18px", borderRadius: 9, border: "none", background: "#2563eb", color: "#fff", fontSize: 14, cursor: "pointer", fontWeight: 500, whiteSpace: "nowrap" },
  btnOutline: { padding: "9px 18px", borderRadius: 9, border: "0.5px solid #d1d5db", background: "#fff", color: "#374151", fontSize: 14, cursor: "pointer", whiteSpace: "nowrap" },
  btnDanger: { padding: "6px 12px", borderRadius: 8, border: "0.5px solid #fca5a5", background: "#fff", color: "#ef4444", fontSize: 13, cursor: "pointer" },
  iconBtn: { padding: "6px 10px", borderRadius: 8, border: "0.5px solid #e5e7eb", background: "#f9fafb", fontSize: 16, cursor: "pointer", lineHeight: 1 },
};